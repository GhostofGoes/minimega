...
type KVMConfig struct {
	...
	// NOTE: the 'migrate' command is **DEPRECATED** and will be removed in a
	// future release. Use 'vm save' instead.
	//
	// Assign a VM state image, generated by a previously saved VM to boot
	// with. Explicitly, this is the state of a previously running VM and NOT
	// the disk state. By default, images are read from the files directory as
	// specified with -filepath. This can be overridden by using an absolute
	// path. State images should be booted with a kernel/initrd, disk, or
	// cdrom. The state often does not make sense without the corresponding disk
	// file and using it with an unrelated disk (or cdrom) is discouraged.
	...
	// Requires QEMU 6.1+ compiled with qemu-vdagent chardev and for
	// spice-vdagent to be installed on VM.
	//
	// Default: false
